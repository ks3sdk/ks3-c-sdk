...
static int init_list_parts(const char *ht, const char *bt, const char *ot, const char* filename, char *up_id) {
    int result = 0;
    ...
    int part_num = 0;
    multiparts_upload(ht, bt, ot, ak, sk, filename, up_id, &result_arr, &part_num, &result, 3);   
    if (result_arr)
        free(result_arr);
    result_arr = NULL;
...
void TEST_LIST_PARTS_ALL_NULL(void) {
    resp = list_parts(NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    CU_ASSERT(NULL == resp);
...
void TEST_LIST_PARTS_RETURNCODE_NULL(void) {
    ...
    resp = list_parts(host, bucket, object_key, ak, sk, NULL, NULL, NULL);
    CU_ASSERT(NULL == resp);

    resp = list_parts(host, bucket, object_key, NULL, NULL, NULL, NULL, NULL);
    CU_ASSERT(NULL == resp);
    
    resp = list_parts(host, bucket, NULL, NULL, NULL, NULL, NULL, NULL);
    CU_ASSERT(NULL == resp);

    resp = list_parts(NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    CU_ASSERT(NULL == resp);
    
    resp = list_parts("", "", "", "", "", "", "", NULL);
    CU_ASSERT(NULL == resp);
...
void TEST_LIST_PARTS_HOST(void) {
    ...
    resp = list_parts("", bucket, object_key, ak, sk, query_str, NULL, &error);
    CU_ASSERT(3 == error);
    buffer_free(resp);
    
    resp = list_parts(NULL, bucket, object_key, ak, sk, query_str, NULL, &error);
    CU_ASSERT(3 == error);
    buffer_free(resp);

    resp = list_parts("ks3-cn-shanghai.ksyun.com", bucket, object_key, ak, sk, query_str, NULL, &error);
    CU_ASSERT(0 == error);
    ...
    resp = list_parts("www.kingsoft.com", bucket, object_key, ak, sk, query_str, NULL, &error);
    CU_ASSERT(0 == error);
...
void TEST_LIST_PARTS_BUCKET(void) {
    ...
    snprintf(query_str, sizeof(query_str), "uploadId=%s", uploadid_str);
    resp = list_parts(host, "", object_key, ak, sk, query_str, NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(404 == resp->status_code);
    if (resp->status_code != 404) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
    ...
    resp = list_parts(host, NULL, object_key, ak, sk, query_str, NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(404 == resp->status_code);
    if (resp->status_code != 404) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
    ...
    resp = list_parts(host, "my-bucket", object_key, ak, sk, query_str, NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(307 == resp->status_code);
    if (resp->status_code != 307) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
...
void TEST_LIST_PARTS_OBJECT(void) {
    ...
    snprintf(query_str, sizeof(query_str), "uploadId=%s", uploadid_str);
    resp = list_parts(host, bucket, "", ak, sk, NULL, NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(200 == resp->status_code);
    if (resp->status_code != 200) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
    ...
    resp = list_parts(host, bucket, "", ak, sk, query_str, NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(404 == resp->status_code);
    if (resp->status_code != 404) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
    ...
    resp = list_parts(host, bucket, NULL, ak, sk, query_str, NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(404 == resp->status_code);
    if (resp->status_code != 404) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
    ...
    resp = list_parts(host, bucket, "123456789", ak, sk, query_str, NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(404 == resp->status_code);
    if (resp->status_code != 404) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
...
void TEST_LIST_PARTS_KEY(void) {
    ...
    resp = list_parts(host, bucket, object_key, NULL, NULL, query_str, NULL, &error);
    CU_ASSERT(-1 == error);
    CU_ASSERT(NULL == resp);
    
    resp = list_parts(host, bucket, object_key, "", NULL, query_str, NULL, &error);
    CU_ASSERT(-1 == error);
    CU_ASSERT(NULL == resp);
    ...
    resp = list_parts(host, bucket, object_key, "", "", query_str, NULL, &error);
    CU_ASSERT(0 == error);
    ...
    resp = list_parts(host, bucket, object_key, "sdfasdfasd", "7s9d7fasdf9asd89fasdfhsd", query_str, NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(403 == resp->status_code);
    if (resp->status_code != 403) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
...
void TEST_LIST_PARTS_QUERYPARA(void) {
    ...
    resp = list_parts(host, bucket, object_key, ak, sk, "", NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(404 == resp->status_code);
    if (resp->status_code != 404) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
    ...
    snprintf(query_str, sizeof(query_str), "uploadId=%s&max-parts=%d&part-number-marker=%d", uploadid_str, max_part, 1);
    resp = list_parts(host, bucket, object_key, ak, sk, query_str, NULL, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(200 == resp->status_code);
    if (resp->status_code != 200) {                        
...
void TEST_LIST_PARTS_HEADERPARA(void) {
    ...
    resp = list_parts(host, bucket, object_key, ak, sk, query_str, "", &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(200 == resp->status_code);
    if (resp->status_code != 200) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
    ...
    resp = list_parts(host, bucket, object_key, ak, sk, query_str, header_str, &error);
    CU_ASSERT(0 == error);
    CU_ASSERT(200 == resp->status_code);
    if (resp->status_code != 200) {                        
        printf("test %s:%d:\n", __FUNCTION__, __LINE__);          
